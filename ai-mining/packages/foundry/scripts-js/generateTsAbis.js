...
function getDeploymentHistory(broadcastPath) {
  ...
  const proxyContracts = new Map(); // Track proxy contracts
  ...
  for (const file of runFiles) {
    ...
    for (const tx of transactions) {
      if (tx.transactionType === "CREATE") {
        ...
        if (tx.contractName && tx.contractName.includes("Proxy")) {
          // Extract the implementation contract name from arguments or context
          // For ERC1967Proxy, the first argument is usually the implementation address
          const implementationAddress = tx.arguments && tx.arguments[0];
          if (implementationAddress) {
            // Find the implementation contract in previous transactions
            const implTx = transactions.find(t => 
              t.contractAddress && 
              t.contractAddress.toLowerCase() === implementationAddress.toLowerCase()
            );
            if (implTx) {
              // Use the implementation contract name but proxy address
              proxyContracts.set(implTx.contractName, {
                contractName: implTx.contractName,
                address: tx.contractAddress, // Use proxy address
                deploymentFile: file,
                transaction: tx,
        ...
        // Store or update contract deployment info, but skip pure proxy contracts
        if (!tx.contractName || !tx.contractName.includes("Proxy")) {
  ...
  // Merge proxy contracts, giving priority to proxy addresses
  const result = Array.from(deploymentHistory.values());
  proxyContracts.forEach((proxyInfo, contractName) => {
    const existingIndex = result.findIndex(item => item.contractName === contractName);
    if (existingIndex >= 0) {
      // Replace with proxy version
      result[existingIndex] = proxyInfo;
    } else {
